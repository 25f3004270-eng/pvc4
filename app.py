...
DATABASE_URL = os.environ.get("DATABASE_URL") or 'mysql+pymysql://root:@localhost/pvc_db'
...
class TenderMaster(db.Model):
    ...
    basic_rate = db.Column(db.Float, default=0)
    pvc_base_date = db.Column(db.String(10))
    supply_date = db.Column(db.String(10))
    call_date = db.Column(db.String(10))
    dp_end_date = db.Column(db.String(10))

    lower_rate = db.Column(db.Float, default=0)
    lower_rate_base_date = db.Column(db.String(10))
...
def get_index_value(idx_df, month, col):
    if idx_df is None or idx_df.empty:
        raise ValueError("Index data not found for item")
    month = to_month_start(month)
    eligible = idx_df[idx_df.index <= month]
    if eligible.empty:
        raise ValueError(f"Index missing before {month.strftime('%Y-%m')}")
    row = eligible.iloc[-1]
    if col not in row or pd.isna(row[col]):
        raise ValueError(f"Index column '{col}' missing for month {month.strftime('%Y-%m')}")
    return to_float_indian(row[col])


def get_igbt_indices(idx_df, base_month, ref_month, currency):
    currency = (currency or "").upper().strip()
    er_col = f"ER_{currency}"

    base_month = to_month_start(base_month)
    ref_month = to_month_start(ref_month)

    base = {
        "C": get_index_value(idx_df, base_month, "C"),
        "AL": get_index_value(idx_df, base_month, "AL"),
        "IM": get_index_value(idx_df, base_month, "IM"),
        "FE": get_index_value(idx_df, base_month, "FE"),
        "W": get_index_value(idx_df, base_month, "W"),
        "D": get_index_value(idx_df, base_month, "D"),
        "ER": get_index_value(idx_df, base_month, er_col),
    }

    curr = {
        "C": get_index_value(idx_df, ref_month - relativedelta(months=1), "C"),
        "AL": get_index_value(idx_df, ref_month - relativedelta(months=1), "AL"),
        "IM": get_index_value(idx_df, ref_month - relativedelta(months=1), "IM"),
        "FE": get_index_value(idx_df, ref_month - relativedelta(months=1), "FE"),
        "W": get_index_value(idx_df, ref_month - relativedelta(months=1), "W"),
        "D": get_index_value(idx_df, ref_month - relativedelta(months=1), "D"),
        "ER": get_index_value(idx_df, ref_month - relativedelta(months=3), er_col),
    }
    return base, curr
...
class SimpleIndices:
    ...
    def ratio(self, key):
        return self.current[key] / self.base[key]

def calculate_igbt_p1(indigenous_value, indices, weights):
    factor = (
        weights.get("FIXED", 16)
        + weights.get("C", 26) * indices.ratio("C")
        + weights.get("AL", 13) * indices.ratio("AL")
        + weights.get("FE", 18) * indices.ratio("FE")
        + weights.get("IM", 9) * indices.ratio("IM")
        + weights.get("W", 18) * indices.ratio("W")
    )
    return (indigenous_value / 100.0) * factor - indigenous_value


def calculate_igbt_p2(cif_value, indices):
    return (cif_value / 100.0) * (
        (indices.current["ER"] / indices.base["ER"]) * (100 + indices.current["D"])
        - (100 + indices.base["D"])
    )


def calculate_igbt_vendor_pvc(rate, vendor_obj, base_month, ref_month, idx_df, weights, scenario):
    base, curr = get_igbt_indices(idx_df, base_month, ref_month, vendor_obj.currency)
    indices = SimpleIndices(base, curr)

    cif = to_float_indian(vendor_obj.cif)
    base_duty = (cif * base["D"]) / 100.0
    basic_for_p1 = rate - cif - base_duty

    p1 = calculate_igbt_p1(basic_for_p1, indices, weights)
    p2 = calculate_igbt_p2(cif, indices)
    total_pvc = p1 + p2

    vendor_row = {
        "scenario": scenario,
        "vendor": vendor_obj.vendor_name,
        "currency": vendor_obj.currency,
        "cif": safe_round(cif),
        "base_duty": safe_round(base_duty),
        "basic_for_p1": safe_round(basic_for_p1),
        "p1": safe_round(p1),
        "p2": safe_round(p2),
    ...
    index_rows = []
    for k in ["C", "AL", "IM", "FE", "W", "D", "ER"]:
        index_rows.append({
            "scenario": scenario,
            "vendor": vendor_obj.vendor_name,
            "parameter": k,
            "base_month": str(base_month.date()) if hasattr(base_month, "date") else str(base_month),
            "base_value": safe_round(base[k]),
            "current_month": str(ref_month.date()) if hasattr(ref_month, "date") else str(ref_month),
            "current_value": safe_round(curr[k]),
        })

    return vendor_row, index_rows


def calculate_igbt_scenario(rate, vendors, base_month, ref_month, idx_df, weights, scenario_name):
    vendor_rows = []
    index_rows = []

    for vendor in vendors:
        vrow, irows = calculate_igbt_vendor_pvc(
            rate=rate,
            vendor_obj=vendor,
        ...
        index_rows.extend(irows)

    min_pvc = min([r["total_pvc"] for r in vendor_rows]) if vendor_rows else 0
    return min_pvc, vendor_rows, index_rows

def calculate_igbt_propulsion_from_tender(item, tender_id, idx_df, weights):
    tender = TenderMaster.query.get_or_404(int(tender_id))
    vendors = TenderVendor.query.filter_by(tender_id=tender.id).all()
    if not vendors:
        raise ValueError("No vendor rows configured for selected tender.")

    basic_rate = safe_float(tender.basic_rate)
    pvc_base_month = to_month_start(tender.pvc_base_date)
    call_month = to_month_start(tender.call_date)
    dp_month = to_month_start(tender.dp_end_date)

    supply_date = pd.to_datetime(tender.supply_date, errors="coerce")
    dp_end = pd.to_datetime(tender.dp_end_date, errors="coerce")

    lower_rate = safe_float(tender.lower_rate)
    lower_base_month = to_month_start(tender.lower_rate_base_date) if lower_rate > 0 else pd.NaT
    lower_rate_applicable = lower_rate > 0 and pd.notna(lower_base_month)

    delay_days = max((supply_date - dp_end).days, 0) if pd.notna(supply_date) and pd.notna(dp_end) else 0
    delay_weeks = math.ceil(delay_days / 7) if delay_days > 0 else 0
    ld_percent = min(delay_weeks * 0.5, 10.0)

    pvc_s1, pvc_s1_vendor, idx_s1 = calculate_igbt_scenario(
        basic_rate, vendors, pvc_base_month, call_month, idx_df, weights, "S1_ORIGINAL_CALL"
    )

    all_vendor_rows = list(pvc_s1_vendor)
    all_index_rows = list(idx_s1)

    if supply_date <= dp_end:
        payable_pvc = pvc_s1
        ld_amount = 0.0
        decision = "S1 only (No delay)"
        selected_scenario = "S1"

    elif not lower_rate_applicable:
        pvc_s2, pvc_s2_vendor, idx_s2 = calculate_igbt_scenario(
            basic_rate, vendors, pvc_base_month, dp_month, idx_df, weights, "S2_ORIGINAL_DP"
        )
        all_vendor_rows.extend(pvc_s2_vendor)
        all_index_rows.extend(idx_s2)

        gst_tmp = (basic_rate + pvc_s1) * 18.0 / 100.0
        ld_base = basic_rate + pvc_s1 + gst_tmp
        ld_amount = round(ld_base * ld_percent / 100.0, 2)

        selected_scenario = "S1" if (pvc_s1 - ld_amount) <= (pvc_s2 - ld_amount) else "S2"
        payable_pvc = pvc_s1 if selected_scenario == "S1" else pvc_s2
        decision = "S1 vs S2 (Delay, no lower rate)"

    else:
        pvc_s2, pvc_s2_vendor, idx_s2 = calculate_igbt_scenario(
            basic_rate, vendors, pvc_base_month, dp_month, idx_df, weights, "S2_ORIGINAL_DP"
        )
        pvc_s3, pvc_s3_vendor, idx_s3 = calculate_igbt_scenario(
            lower_rate, vendors, lower_base_month, call_month, idx_df, weights, "S3_LOWER_CALL"
        )
        pvc_s4, pvc_s4_vendor, idx_s4 = calculate_igbt_scenario(
            lower_rate, vendors, lower_base_month, dp_month, idx_df, weights, "S4_LOWER_DP"
        )

        all_vendor_rows.extend(pvc_s2_vendor + pvc_s3_vendor + pvc_s4_vendor)
        all_index_rows.extend(idx_s2 + idx_s3 + idx_s4)
        ...
        selected_scenario = min(compare, key=compare.get)
        payable_pvc = {"S1": pvc_s1, "S2": pvc_s2, "S3": pvc_s3, "S4": pvc_s4}[selected_scenario]
        decision = f"{selected_scenario} selected (Lower-rate applicable)"

    result_row = {
        "pvc_actual": safe_round(pvc_s1),
        "pvc_contractual": safe_round(payable_pvc),
        "lower_actual": None,
        "lower_contractual": None,
        "delay_days": delay_days,
        "ld_weeks_new": delay_weeks,
        "ld_rate_pct_new": safe_round(ld_percent),
        "ld_applicable": delay_days > 0,
        "ld_amt_actual": safe_round(ld_amount),
        "ld_amt_contractual": safe_round(ld_amount),
        "pvc_actual_less_ld_new": safe_round(pvc_s1 - ld_amount),
        "pvc_contractual_less_ld_new": safe_round(payable_pvc - ld_amount),
        "lower_actual_less_ld": None,
        "lower_contractual_less_ld": None,
        "fair_price_new": safe_round(payable_pvc),
        "selected_scenario_new": selected_scenario,
        "scenario_amounts": {
            "S1": safe_round(pvc_s1),
            "FINAL": safe_round(payable_pvc),
        },
        "scenario_details": all_index_rows,
        "vendor_details": all_vendor_rows,
        "decision": decision,
...
def calculate_for_item(item, one, index_df, weights):
    ...
    if code == "IGBT_PROPULSION_SYSTEM":
        tender_id = one.get("tender_id")
        if not tender_id:
            raise ValueError("Tender is required for IGBT Propulsion System.")
        return calculate_igbt_propulsion_from_tender(item, tender_id, index_df, weights)
...
def calculate():
    ...
    data = {
        ...
        'tender_id': request.form.get('tender_id') or '',
    }
